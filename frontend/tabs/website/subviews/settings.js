...
export default async function render(container, ctx = {}) {
    ...
    const groupKeys = (prefix) =>
        Object.keys(_settings).filter(k => k.startsWith(prefix + '.')).sort();
    ...
    container.innerHTML = `
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(400px,1fr));gap:12px;margin-bottom:16px;">
            ${_GROUPS.map(g => _renderGroup(g, groupKeys(g.prefix))).join('')}
        ...
        <div class="card" style="display:flex;gap:8px;align-items:center;">
            ...
            <span style="color:#888;font-size:11px;margin-left:auto;">新增 key 請透過 API 直接操作；本面板僅編輯既有 key。</span>
        </div>
    `;
}
...
function _renderField(key, val) {
    ...
    const inputVal = typeof val === 'string' ? val : JSON.stringify(val ?? '');
    ...
    const control = isLong
        ? `<textarea data-key="${esc(key)}" rows="3" style="width:100%;">${esc(inputVal)}</textarea>`
        : `<input data-key="${esc(key)}" type="${isSecret ? 'password' : 'text'}" value="${esc(inputVal)}" style="width:100%;" />`;
    return `
        <div>
            <label style="color:#888;font-size:11px;display:block;margin-bottom:3px;" title="${esc(key)}">${esc(shortKey)}${isSecret ? ' 🔒' : ''}</label>
            ${control}
        </div>
    `;
}
