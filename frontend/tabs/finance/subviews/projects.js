 ...
 * 存檔後端回算好的值回來，這裡只顯示。
 *
 * 2026-09-13 拆檔：私帳 ↔ 母帳推送（連結鈕／跳母帳／推送彈窗）住 projects-push.js；
 * 本檔 `export let _detail`（live binding，子模組只讀）＋ `setDirty()`／`resetDetail()`。掃原始碼的測試看那一段要指那支檔。
 */
import { finFetch, finIsMine as _isMine, esc, fmtNum, finToast, todayStr }
    from '../fin-utils.js';
import { crmFetch, setupResizeHandle } from '../../crm/crm-utils.js';
// 私帳 ↔ 母帳推送段（2026-09-13 從本檔切出）：循環 import 只在函式內用，模組頂層不碰對方
import { _closeLocked, _linkBtnHtml } from './projects-push.js';

let _c = null;
let _isCurrent = () => true;
let _data = null;
export let _sel = null;        // 目前選取的專案 id（projects-push.js 只讀）
export let _detail = null;     // 右側載入的單案資料
let _q = '';
...
let _dirty = false;
/** 子模組（projects-push.js）改狀態走這兩支 —— ES module 的 import 是唯讀 live binding，子模組直接賦值是 TypeError。 */
export function setDirty(v) { _dirty = !!v; }
export function resetDetail() { _sel = null; _detail = null; _dirty = false; }
            ...
            <div style="display:grid;grid-template-columns:${
                ...
                <div>
                    ...
                    <table class="crm-table" style="width:100%;font-size:12px;">
                        ...
                        <tr><td style="color:#bbb;">結案日${!_closeLocked(p) ? '' : `
                                <span style="font-size:10px;color:#6b7280;margin-left:4px;"
                                      title="以母帳為準：到母帳那一案改，會自動同步過來">母帳</span>`}</td>
                            <td style="width:130px;"><input type="date" class="crm-input fpl-num" id="fpl-close"
                                value="${esc(p.close_date || '')}" style="width:100%;"${_closeLocked(p) ? ' disabled' : ''}></td></tr>
...
_fp.save = async (btn) => {
    ...
    // 連著母帳的案結案日由母帳決定（後端 409）—— 鎖住的格子不送
    if (dEl && !dEl.disabled) body.close_date = dEl.value || '';     // 空＝清成未結案
