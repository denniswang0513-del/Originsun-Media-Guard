...
import { finFetch, finIsMine as _isMine, esc, fmtNum, finToast, todayStr }
    from '../fin-utils.js';
import { crmFetch, searchableSelect, setupResizeHandle } from '../../crm/crm-utils.js';
...
const _closeLocked = (p) => ((p && p.locked_fields) || []).includes('close_date');

/** 動作列上「跟母帳的關係」那一顆：已連結→跳過去；未連結→推送。 */
function _linkBtnHtml(p) {
...
async function _pushSubmit(btn, mode) {
    ...
    if (mode === 'move') {
        ...
        try {
            ...
            _sel = null; _detail = null; _dirty = false;
            document.getElementById('fpl-detail').style.display = 'none';
            await _fp.refresh();
        } catch (e) {
            btn.disabled = false;
