...
// 2026-09-12 拆檔：五段住在 crm-cashbook-{batch,fields,import,alloc,petty}.js（它們 import 這裡的狀態，這裡 import 它們的函式；
// 循環只在函式內用）。掃原始碼的測試用 _srcscan.cashbook_src() 把六支串起來。
import { _bankOut, _batchApply, _batchPaint, _batchPick, _batchRefreshBar, _batchSetMode, _batchTaxDraw, _cardAmt } from './crm-cashbook-batch.js';
import { _bankAcctOpts, _buildEditFields, _invOptsHtml, _invoiceCandidates, _projOptsHtml } from './crm-cashbook-fields.js';
import { _initColumnChooser, _setCsvFile, closeRecon, doImport, openImportModal, openRecon } from './crm-cashbook-import.js';
import { _ALLOC_SIDES, _allocBody, _allocRemain, _renderEmptyPayBox, loadCashInvoiceAllocs, loadCashPaymentAllocs } from './crm-cashbook-alloc.js';
import { _pettyMenu, _pettyTag } from './crm-cashbook-petty.js';
import { indexTax as _indexTaxShared, taxKidsAt as _kidsAt, taxSelects }
    from '../../js/shared/cash-tax-picker.js';

export let _entries = [];
export let _invoiceList = [];
export let _paymentList = [];   // 請款單（付款側的分配用）
export let _projectList = [];
let _clientList = [];
export let _bankAccounts = null;   // 財務模組銀行帳戶；null = 載入失敗/未啟用（優雅降級：不顯示帳戶欄）
...
export async function loadEntries({ render = true, cards = true } = {}) {
...
export async function _loadLinkLists() {
 ...
 *  可搜尋升級，其餘參數原樣往下傳。 */
export function _taxSelects(box, o) {
    taxSelects(box, {
        ...o,
        tree: _taxTree,
...
// 重畫，於是每個動作都要等一秒以上。
const _PAGE = 200;
export let _shown = [];      // 目前這批排序/篩選後的完整列表（畫多少由 _drawn 決定）
let _drawn = 0;
...
async function _cashKaiRemit(e, pids, pnames, kai) {
    ...
    if (gb) gb.addEventListener('click', async (ev) => {
        ...
        ov.remove();
        crmToast(`已開 ${ok} 張代開請款單` + (fails.length ? `；${fails.length} 張失敗：${fails.join('、')}` : ''), fails.length > 0);
        crmCacheInvalidate();
        loadEntries({ cards: false });
    });
}
...
async function _cashPayForProject(e, pid, pname, o = {}) {
    ...
    // 案可複數：每個案一區，各開各的單。（發票代開列不走這裡 —— 那是整筆請過去，見 _cashKaiRemit）
    ...
    const head = `收到 $${_fmtNum(e.deposit || 0)}（${_esc(e.summary || '')}）。`;
    const notes = done.length ? `<div style="color:#f59e0b;font-size:12px;margin-top:4px;">案子裡的錢已請過：${done.length} 人 $${_fmtNum(doneSum)}（已付 ${donePaid}）${done.length === groups.length ? ' —— 執行人員都請過了，沒有可再請的。' : '，下面已灰掉、不能再勾。'}</div>` : '';
...
export const _grossOut = (e) => (e.expense || 0) + (e.bank_fee || 0);
...
export let _CATEGORIES = ['水電網路', '交際應酬', '行政', '其他', '其他收入', '房租', '建構',
