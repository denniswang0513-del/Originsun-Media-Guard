...
> **版本**: v2.5.47（2026-09-17）<!-- publish_update.py 自動維護，勿手改 -->
...
閘門：根因或重現「不確定」、影響對外面／OTA／schema、跨檔搬碼、改公開介面、diff > 40 行、撞「不要動的地方」、
要改測試才綠 → **不動**。**紅→綠硬規定**：沒看到測試先失敗不准改碼。一張卡一個 commit。
**卡片是 Claude 自己的紀律，不是給 owner 審的**：過閘門就修、沒過就不動，全程不問；報告正文只列「修了／沒動」
一行一條，完整卡放附錄。owner 明說要的是「仔細評估、不亂動」，不是「我來批」。
/polish 階段一與 /health 階段一都經過它；owner 說「修這個」也先寫卡再修（同一輪內做完），說「直接修」才跳過卡片（但不跳過紅→綠）。
...
| [`core/office_settings.py`](core/office_settings.py) | 發版時送去 NAS 的設定**允許清單**（發票根目錄／代開費率／申請人／圖床／磁碟對映）＋ `EXPORT_SUBKEYS` 子鍵投影（`company` 只送 name／tax_id／bank／account_name／account_no） | 允許清單不是排除清單（新機密欄位預設不外送）；整個 key 會夾帶東西時改用子鍵投影，別為了一個欄位把整包送過去；代價是 master 改了要等下次發版 |
| [`core/invoice_share.py`](core/invoice_share.py) | 發票分享頁「能出現什麼」的純規則：白名單投影、分享時定稿的快照、作廢判定 | 無 I/O；`SNAPSHOT_FIELDS` 是白名單、`NEVER_SHARE` 給測試逐欄釘住；**作廢走即時值不進快照**；`is_voided` 只認「作廢」一個字（`issue_status` 是三值的）；「匯款資訊」（`remit_view`，2026-09-12）跟頁尾賣方一樣從 `settings.company` 即時讀、不進快照，**作廢不給**，存摺影本只出布林不出路徑 |
...
| `routers/crm/invoice_files.py` 的對外段 | `/e/{短碼}` 那頁的三支公開端點（meta／download／舊長網址）＋ 分享時定稿 | 掛 `public_router`＝NAS 對外容器也吃得到；**永遠 attachment 不 inline**；路徑要過 `drive_map.to_local_path` 才比白名單；開不到檔就 422，不要鑄一條下載會 404 的連結。存摺影本（`/bankbook`＋管理員的 `POST/GET /invoices/bankbook`）放**發票根目錄** `_公司/存摺影本.<ext>`、走同一份 `_local_invoice_path` 白名單 —— 不放 master 的 `company_assets/`（NAS 看不到，master 關機那顆鈕就 404）；**位置固定且不進設定**（`_bankbook_local_path()` 直接 glob 固定位置；第一版存 `company.bankbook_path` 再靠 /publish 送去 NAS，換副檔名後 NAS 會指著已刪的舊檔——設定沒有承擔任何事，/polish 拿掉），所以兩台上傳完立刻生效 |
...
| [`core/reminder_logic.py`](core/reminder_logic.py)＋[`frontend/js/shared/ts-zone/remind.js`](frontend/js/shared/ts-zone/remind.js)（`/me/reminders`，`routers/api_me.py`） | 「要補填」提醒條（owner 2026-09-13）：近 30 天工作日一列都沒有＝沒填、只有草稿＝未完成；近 6 個過完的週週記沒送出。純規則在 core（到職日起、週末／假日表／已核准的假／今天不算），端點只撈資料、**只對在職（`is_active_staff`）回東西**，週記那半邊要 `journal` 鑰匙 | 畫在分頁鈕列上面（`mountZone` 插 `#z1-remind`），每次 `loadLog` 順便重抓＝填完就消失；`day-goto` 從別的視圖也切得到；`journal-goto` 交宿主 `hooks.journalGoto`（員工頁：展開第二區、postMessage `journal-goto-week` 給內嵌 journal.html，它送出後回 `journal-submitted`）。今天那條原本的「未完成」併進來（`mineIncomplete` 端點表拿掉；`/timesheets/mine/incomplete` 仍在） |
| [`core/hr_logic.py::burn_rate`](core/hr_logic.py)＋[`ts-projects.js::budgetChip／burnChip`](frontend/js/shared/ts-projects.js) | 專案檔案的「預算」與「消耗率」一顆各一（owner 2026-09-13）：基準＝手動預算 > 公式預期製作時數 > 算不出；超過 100% 寫「超支 +N h」 | 公式時數（`_suggested_for`）2026-09-13 起專案檔案人人拿得到（只是時數）；`/summary`／`/me/projects_burn` 那張表 2026-09-14 起也走 `burn_rate`（`burn_rows` 回 `base`／`base_hours`，在 `SUMMARY_PUBLIC_KEYS`），仍抹 `suggested_hours` 與錢三欄；清單那格公式值標「公式」。改預算的 ✎ 在格子裡（同 `data-ts-action="budget"`，清空＝回到公式），動作列的「改預算」拿掉 |
| `frontend/tabs/crm/crm-projects-core.js` 的清單「帳本」欄（`_ledgerCellHtml`／`_ledgerSortKey`）＋詳情彈窗（`selectProject` 加 `.as-modal`、`_detailBackdrop`／`_detailEsc`） | 已連結私帳／後期代開／現金收款／私帳／後期專案的標籤自己一欄、可排序（owner 2026-09-14）；專案詳情同一個 `#proj-detail-panel` 節點加 `.as-modal` 蓋在清單上（inset 3vh/3vw，z-index 900 < 各 overlay），backdrop 點了／Esc 關 | 八個分頁的 DOM／id 一個都沒動，其他模組照舊往 `#proj-detail-*` 畫；Esc 判「上面有沒有別的 overlay 開著」要看 computed display，不比 style 字串（#proj-modal 起初 `display:none;` 沒空格、#proj-mirror-modal 關了還在 DOM） |
| 行事曆（owner 2026-09-14，[`docs/CALENDAR_PLAN.md`](docs/CALENDAR_PLAN.md)）：[`core/schedule_logic.py`](core/schedule_logic.py)、[`services/calendar_sync.py`](services/calendar_sync.py)、[`routers/api_calendar.py`](routers/api_calendar.py)、[`frontend/js/shared/calendar/`](frontend/js/shared/calendar/index.js) | 統一事件流 `/api/v1/calendar/events`（拍攝／工作登記／已核准的假／假日／里程碑；scope=me 多帶計畫卡）＋唯一可登記的「工作登記」`crm_schedule`（專案×日期×人員×職務，attendees 可含**外部人員**：沒 staff_id 只有名字，不建檔）＋四種事件同一條 Google 同步線（顏色六類別可調 `google_calendar.colors`，Google 11 色）。三個宿主掛同一份元件：公布欄子視圖、專案詳情第九分頁（scope 鎖 project:）、手機行事曆四段（今天／本週／拍攝／登記工作） | 讀＝`bulletin`／`crm_projects`／`timesheets`／`me_today_zone`／`me_team_week` 任一；登記自己＝綁定人員檔案；排別人或外部人員＝`crm_projects`；顏色／全部重新同步＝admin。**不做任何推播**（owner：排別人不用通知，Google 日曆就是通知）。場次同步仍只從 `api_shoots._finish` 進（測試釘住），calendar_sync 只在 resync-all 叫它；里程碑／假的同步掛在各自寫入端點 commit 之後（`save_week` 只能有一個 commit）。衝突（請假／別場 crew／已排工作）只提醒不擋。`services/calendar_sync._project_names` 是 id 反查，登記在 `test_money_visibility` 的 EXEMPT |
...
| [`routers/timesheets/`](routers/timesheets/_shared.py) | 工作追蹤 API（2026-09-12 從 `routers/api_timesheets.py` 1,100 行拆成套件）：`_shared`（router／守衛／`_CANDS_CACHE`／`SUMMARY_PUBLIC_KEYS`）＋ `sync`／`mine`／`ledger`／`projects`／`reports`／`mapping`／`summary` | `routers/api_timesheets.py` 是薄殼（掛 router、star re-export 端點給直接呼叫的測試）；**monkeypatch 要打子模組**，patch 薄殼沒用。掃原始碼用 `_srcscan.timesheets_src()`。路由集合跟拆前逐一比對相同（47 支） |
...
| [`routers/crm/costs.py`](routers/crm/costs.py) ← [`cost_groups.py`](routers/crm/cost_groups.py) ← [`cost_lines.py`](routers/crm/cost_lines.py) | 2026-09-13 從 1,784 行拆三支（純搬移）：costs＝雜支登記（含分享連結）／收據儲存／財務摘要＋四支共用小 helper（`_fmt_date`／`_get_first_group_id`／`_resolve_target_group`／`_project_misc_pct`）；cost_groups＝成本子表；cost_lines＝成本估算＋範本＋從報價單匯入 | 分層無循環（只往左 import）；`petty`／`project_links` 的 `from .costs import _resolve_target_group` 不用改；掃原始碼用 `_srcscan.costs_src()`（三支串起來） |
| [`routers/crm/projects.py`](routers/crm/projects.py) ＋ `project_closing.py`／`project_import.py`／`project_links.py`／`project_map.py` | 2026-09-13：projects（清單／建改刪／狀態／單筆 wire）、project_closing（結案看板 `/projects/closing`／官網階段／工項；`_CLOSING_STATUS`／`_WEBSITE_PROD_STAGES` 正本）、project_import（CSV 匯入）、project_links（換帳本／推送／收款方式／連結寫入 `_write_link`＋`_sync_pair`＋`_crm_client_for`）、project_map（母私帳對應表五支端點） | 🔴 `/projects/closing` 必須先於 `/projects/{project_id}` 註冊 —— 靠 **projects.py 檔頭 import project_closing**（decorator 先跑），不是 `__init__` 的順序；project_closing 因此不准 import projects。`__init__` 順序：projects → project_links → project_map → project_import。掃原始碼用 `_srcscan.projects_src()`（五檔） |
...
| `core/overtime_logic.py` ＋ `routers/api_overtime.py` ＋ `js/my/cards-ot.js`／`m/views/leave.js`／`tabs/hr_leave`（加班佇列） | **加班申請**（docs/PAYROLL_OVERTIME_PLAN.md 第二批，2026-09-18）：員工填日期、起訖、換補休或加班費、事由；時數與工作日／假日後端算；核准 → 補休進時數帳（平日 1:1 假日 1:2、當年 12/31 到期）或加班費（公司規定 工作日 ×1 假日 ×2）掛加班日那個月的薪資單草稿（已確認就往後找） | 員工端 `require_bound_staff(request,'me_leave')`、不收客戶端時數；管理端清單 hr_leave／finance_partner、核准退回 check_admin；每月 46 h 黃、54 h 擋；`api_overtime.router` 是兩支合成的，include 一定在端點之後；NAS office-api 也掛（手機要能報） |
...
- **推送「不擋 0」只放行新建，連既有私帳案時 CRM 算出 0 要退成 keep**（2026-09-12 /polish review 抓到）：沒綁人員檔案或成本行一筆都沒掛給我時 `mir["total"]` 是 0，`overwrite` 會把他親手填的合約額與工項洗成 0、`add` 會清掉 `mirror_total`；`mirror_project_to_mine` 在連既有案時把 overwrite/add 降成 keep（代開發票例外：那案收入本來就不是成本行），回傳的 `mode` 是**實際套用的**。同一個原因：沒綁人員檔案時 `stale` 一律 None（0 是「認不出來」不是「沒有」，拿去比會把每一案都標成落後）。
- **母帳 PUT 親手送上來的欄位不做「母帳空白就拿私帳的補」**（`_sync_pair(no_fill=set(update_data))`）：清結案日＝重開案，同一交易被私帳補回去等於存不進去而且回應還說改好了。**私帳客戶 `crm_link_id` 指到母帳客戶時視為同一人**（`skip`）——不然推送到母帳的當下私帳案的客戶被換成母帳那筆，私帳自己的客戶檔／匯款資訊從此對不上。
- **私帳結案日的 409 牆只在值真的變時擋**（`raw != _fmt_day(p.completion_date)`）：CF 給 .js 4 小時快取，舊分頁整包送 close_date（沒動也送），看到鍵就擋＝連著母帳的案四小時內一格都存不了。
- **`routers/crm/projects.py` 2026-09-12 拆成兩支**（它曾到 1,979 行，離 `test_files_stay_readable` 的 2,000 上限 21 行）：換帳本／推送／對應表住 `routers/crm/project_links.py`（`routers/crm/__init__` 在 projects 之後 import，路由才註冊）；連結的**判定**（`is_mirrored`／`resolve_mine_link`／`mine_link_map`）留在 projects.py。兩檔互相依賴：project_links 頂層 import projects，projects 的 `update_project` 用 `_sync_pair` 走**函式內** lazy import。掃原始碼的測試用 `_srcscan.projects_src()`（兩檔串起來），別指單一檔；`from routers.crm.projects import X` 的 X 若是搬走的（`_LEDGER_BLOCKERS`／`MIRROR_MODES`／`_mirror_*`／`_blocked_reason`）要改 import `project_links`。
- **母帳親手清空的識別欄，私帳跟著清**（`sync_from_parent(explicit=)`；收尾 review 抓到）：只擋「補回母帳」不跟著清的話，連結中的私帳案永遠重開不了（私帳側 409、母帳側清了不流過去）。反向補到結案日時母帳狀態進「結案」並走 `_apply_status_side_effects`（不然有結案日卻不是結案，挑案器照樣當進行中）。N:1 判定與私帳詳情鎖欄位同一份（`mine_parent_links`，母帳案已刪的舊指標不算）。
- 🔴 **dev 8001 的「幽靈 LISTEN」真相**（2026-09-12 咬了一整晚）：`Stop-Process` 殺掉 uvicorn 的 reload 子行程後，它用 `multiprocessing.spawn` 生的孫行程（system python，`-c "from multiprocessing.spawn import..."`）**還活著並繼承了 8001 的 socket**，之後每次「重啟」都多一隻，請求輪流落到跑舊碼的孫行程上——症狀是「同一支腳本時好時壞」、加了 log 卻沒寫。殺法：`Get-CimInstance Win32_Process -Filter "Name='python.exe'"` 挑 CommandLine 含 `multiprocessing.spawn` 且父行程已死的全殺，`netstat -ano | findstr :8001` 只剩一個 LISTENING 才算乾淨（8000 的 pythonw 別碰）。
...
- **「代辦費已扣除」是應收的旗標，不是營收的**（owner 2026-09-13 第 1 點「應收不再減一次，但是營收是合約金額，只是應收先扣掉了」）：
  ...
  `payout_total` 2026-09-13 起也看它（owner「不用再匯，因為已經有這個支出付掉了」）：已扣時稅金＋買發票不列成要匯出去的（`FEE_PARTS`），取消已扣才算。
  切成後期代開時分身收入**一律**＝母帳合約額（owner「一律改成合約」；母帳沒填才留原值）。
- **拆 ES module 時，每一個從主檔拿的名字都要進 import 清單 —— 用 grep 查程式碼，別信「看起來只在註解裡」**（2026-09-13 拆 `projects-push.js`
  漏了 `_sel`）：eslint 關了 no-undef、`node --check` 也過、單元套件全綠，只有真瀏覽器按「推送到母帳」才 `_sel is not defined`。
  拆 JS 之後**一定**跑一輪 Playwright 把拆出去那段的每顆鈕按過（`ui_push.py`／`ui_billing.py` 那種）。同一天拆 Python 三次都用
  ruff F821 抓到漏名（`_crm_client_for`／`not_mine`／`_hide_mine`／`_check_auth`）—— Python 有靜態閘門，JS 沒有。
- **`/projects/closing` 的路由順序靠 import 順序**（2026-09-13 拆 `project_closing.py`）：那支必須註冊在 `/projects/{project_id}` 之前，
  否則 "closing" 被吃成 project_id。做法是 projects.py **檔頭** import project_closing（它的 decorator 先跑）；改成在 `__init__` 排順序
  或讓 project_closing 反向 import projects 都會壞（後者成環）。拆任何帶 `/{id}` 萬用路由的檔案前先列出同前綴的靜態路徑。
...
- **行事曆的「做了 ✓」→ 工時列只有一條路**（2026-09-14）：計畫卡（plan_row_id）還在就把那張卡變實際（同一列），不然才 `add_rows` 插新列；
  `timesheet_ids` 記住產過的，第二次按回同一列。手機端 UI 文字不准有 emoji／✓（`test_no_emoji_in_ui_text`），狀態字從 options 推。
- **我的一週「加一項」不掛 Enter 的 keydown**（2026-09-14）：表單 `onsubmit="return false"` 已擋送出；再 `preventDefault` 會把專案格
  `<input list>` 的「Enter 選中 datalist 建議」吃掉（Chrome 的 datalist 選取是 Enter 的預設動作）。建卡只走「加入」鈕。
- **「要補填」提醒只認狀態正好是「在職」**（owner 2026-09-14）：不用 `is_active_staff`（那把尺把合夥與空白也算在職，是守衛用的）。
...
- **`login-modal.js` 那份補 token 的 `fetch` 只認幾個路徑前綴**（`/settings/`、`/admin/`、`/auth/`…），`/api/v1/crm/` **不在裡面**：
  非 module 的設定視窗（`settings-modal.js`）打 CRM 端點要自己帶 `window.bearerHeader()`，不然症狀是「上傳失敗：未登入或 token 已過期」
  而人明明登入著（2026-09-12 存摺影本上傳就這樣踩到）。要加路徑進那份白名單也行，但先看它是怎麼靠 method 分流的。
- **「免登入分享頁的下載來源」不要做成設定鍵**（2026-09-12 /polish）：存摺影本第一版存 `settings.company.bankbook_path`，
  於是 `/api/settings/save` 得多守一個子鍵（非管理員指到別張發票＝把它公開送出去）、NAS 要等發版才拿到、換副檔名後 NAS
  指著已刪的檔。全公司一份、檔名固定的東西，**固定位置就是正本**，不需要任何設定。
