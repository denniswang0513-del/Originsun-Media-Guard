...
def test_partner_default_is_everything_but_private_ledger():
    ...
    assert p == set(ALL_MODULES) - {"finance_mine", "finance_partner", "me_todos", "me_finance", "knowledge"}, "其餘全部（私帳、與 money_view 互斥的母公司報表唯讀鍵、畫面未開的兩張卡、owner 自己勾的知識庫除外）"
    assert not any("knowledge" in DEFAULT_TEMPLATES[i] for i in IDENTITIES), "知識庫三個身份都不預設配（owner 2026-09-18）"
