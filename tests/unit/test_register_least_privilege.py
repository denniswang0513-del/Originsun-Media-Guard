...
2. me_profile 只開個人資料卡：「今天與這週」那一區與它後面的端點要的是 ME_WORK_KEYS（不含 me_profile）。
...
def test_profile_key_is_not_a_work_key():
    from core.auth import ME_MODULE_KEYS, ME_WORK_KEYS
    assert set(ME_WORK_KEYS) == set(ME_MODULE_KEYS) - {"me_profile"}
    assert "me_profile" in ME_MODULE_KEYS, "me_profile 仍是 workspace 的鑰匙（allowed 要帶它，卡片才畫）"


def test_today_zone_endpoints_require_a_work_key():
    """/me/today、/me/team_week、/timesheets/mine*：綁了人員檔案、只有 me_profile 的帳號也進不去。"""
    me = code_only(repo_src("routers/api_me.py"))
    assert "require_bound_staff(request, *ME_WORK_KEYS)" in func_body(me, "async def _me_bound(")
    ts = code_only(repo_src("routers/api_timesheets.py"))
    assert 'require_bound_staff(request, "timesheets", *ME_WORK_KEYS)' in func_body(ts, "async def _mine_ident(")
    # 兩支 router 都不再拿 ME_MODULE_KEYS 當守衛鑰匙（workspace 的 allowed 計算除外）
    assert "require_bound_staff(request, *ME_MODULE_KEYS)" not in func_body(me, "async def _me_bound(")
...
def test_workspace_draws_the_profile_card_and_hides_the_today_zone_for_profile_only():
    ...
    assert 'ws.allowed.some(k => staffSections.includes(k) && k !== "me_profile")' in html, \
        "「今天與這週」不能因為 me_profile 就出現（後端 ME_WORK_KEYS 同一條）"
