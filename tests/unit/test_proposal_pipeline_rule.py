# -*- coding: utf-8 -*-
"""「這筆提案該不該補殼專案」—— `api_proposals.needs_shell_backfill` 的正反面。
...
def test_pipeline_less_proposals_get_a_shell(status):
    assert needs_shell_backfill(NS(project_id=None, status=status)) is True


def test_the_exempt_status_stays_out():
    """已停擺的提案刻意不進管線洗版 —— 這是唯一的豁免。"""
    assert needs_shell_backfill(NS(project_id=None, status=PIPELINE_EXEMPT_STATUS)) is False
...
def test_already_linked_never_gets_a_second_shell(status):
    ...
    assert needs_shell_backfill(NS(project_id="p1", status=status)) is False
