...
(() => {
    ...
    window._activeRemoteHosts[HOST_A] = {
        ...
        destBase: '//nas/proxy/PROJ/HostDispatch_HostA',
        state: 'running',
        pct: 40,
        // 已經超過失聯門檻 —— 不必真的等 90 秒
...
def _state(page):
    return page.evaluate("() => ({ calls: window.__calls, hosts: window._activeRemoteHosts })")
...
def test_dead_host_work_is_reassigned(page):
    ...
    page.evaluate("() => { if (window._heartbeatTimer) clearInterval(window._heartbeatTimer); }")
    ...
    blacklist = page.evaluate("() => window._retryFailedHosts")
    assert "10.99.0.1:8000" in blacklist
