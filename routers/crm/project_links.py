"""routers/crm/project_links.py — 母帳 ↔ 私帳：換帳本、推送（分身）、收款方式、連結的寫入（_write_link／_sync_pair）。

2026-09-12 從 projects.py 原樣切出（純搬移；那檔 1,979 行離單次讀取上限 21 行）；
2026-09-13 再把對應表五支端點切去 project_map.py（分層 projects ← project_links ← project_map，無循環）。
...
projects.update_project 要用這裡的 _sync_pair／apply_billing_mode，走函式內 lazy import（兩檔互相依賴）。
掃原始碼的測試用 `_srcscan.projects_src()`（三檔串起來）。
"""
...
from .projects import _apply_status_side_effects, resolve_mine_link
...
async def check_project_mirror(project_id: str, request: Request):
    ...
    async with factory() as session:
        ...
        if linked is None:
            ...
            shared = n_parents > 1 and share is None
            if not shared:
...
async def _clear_pending_if_all_claimed(session, t, keep: dict) -> dict:
    ...
    from ._shared import mine_parent_links
    if keep.get(BY_PARENT_PENDING_KEY) is not True:
        return keep
    parents = {pid for pid, _n in (await mine_parent_links(session, [t.id])).get(t.id) or []}
    if parents and parents <= set(parent_shares(keep)):
        keep.pop(BY_PARENT_PENDING_KEY, None)
    return keep
...
async def mirror_project_to_mine(project_id: str, req: ProjectMirrorPayload,
    ...
    from core.ledger import require_entity
    from core.ledger_project import (BILLING_MIRROR_SOURCE, BY_PARENT_PENDING_KEY, MIRROR_AT_KEY, MIRROR_SOURCE, MIRROR_TOTAL_KEY,
                                     SELECTABLE_SOURCES, apply_source_fee, billing_mode_of, merge_split, parent_shares,
                                     set_parent_share,
                                     norm_detail)
    from routers.api_finance_projects import resync_receivable
    ...
    async with factory() as session:
        ...
        if target_id:
            ...
            if mode == "import":
                ...
                # 分案記帳（owner 2026-09-13「為何不加起來？」）：只換**這一個母帳案**的份額，
...
async def apply_billing_mode(session, p, request, old_mode: str) -> dict:
    ...
    share = {"amount": 0, "split": {}}
    if t is not None:
        keep = await _ensure_share(session, t, keep, p.id)
        share = parent_shares(keep).get(p.id) or share
...
async def _write_link(session, parent, mine):
    ...
    from core.ledger_project import (BILLING_MIRROR_SOURCE, BY_PARENT_PENDING_KEY, MIRROR_SOURCE, billing_mode_of,
