...
def _picker_view(opt: dict) -> dict:
    ...
    🔴 `client` 是客戶的**顯示名**：代稱優先、沒填代稱才退到全稱（`Client.full_name`，
    也就是發票抬頭）—— 所以這支免登入端點上**可能出現客戶全稱**。判斷是「跟這支已經
    在露的東西同一級」：案名（含私帳案）本來就在上面，而不給客戶名就分不出兩個同名的案
    要選哪一個。要收緊就改 `services/project_picker` 那支 coalesce，別只改這裡的字。
    `client_id`／`entity`／任何金額欄一律不在這裡。
    """
    return {
        "id": opt["id"],
