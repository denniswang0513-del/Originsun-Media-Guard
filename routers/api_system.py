...
# （存摺影本刻意**不是**設定鍵：免登入分享頁的下載來源若能從設定指，就得再守一次 —— 它是固定位置的檔，見 invoice_files._bankbook_local_path）
...
#    `knowledge.root`＝知識庫（owner 的書）住在哪顆磁碟（docs/KNOWLEDGE_BASE_PLAN.md §11）—— 同一條理由。
_SECRET_SUBKEYS = {"google_oauth": ("client_secret",), "google_calendar": ("service_account_json",),
                   "finance": ("fortress",), "knowledge": ("root",)}
