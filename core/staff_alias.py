...
3. **收款人那段字「包含」某個姓名**，而且只包含一個人的 → 就是他。
4. 都沒有才輪到「包含某個代稱」，一樣只包含一個人的 → 就是他。
   第 3、4 條專門對付「用途 ＋ 人名」那種寫法。
5. 其餘 → 對不到，照原字串走（畫面上要明說「還沒對到人員」，不是留白）。

🔴 **姓名要排在代稱前面**（2026-09-11 /polish 抓到）：只比代稱的話，
   「停車費 史丹利」會落到代稱是「史丹」的**另一個人**身上 —— 因為「史丹」
   是「史丹利」的子字串。撈到的是錯的銀行帳號與身分證，匯款通知也掛在錯的
   人名下，而畫面上完全看不出異狀。兩條都是逐字包含、都不是模糊比對。
...
def resolve(payee: str, index: dict):
    ...
    if s in by_alias:
        return by_alias[s]
    # 第 3、4 條：包含。姓名先看 —— 見檔頭「姓名要排在代稱前面」。
    # 兩個字以上才算「包含」：單字姓名（或單字代稱）當子字串會亂咬。
    # 各自撞到兩個不同的人就放棄（寧可對不到）。
    for table in (by_name, by_alias):
        hits = {key for k, key in table.items() if k and len(k) >= 2 and k in s}
        if len(hits) == 1:
            return hits.pop()
        if hits:
            return None       # 這一層就已經分不清了，不要退到下一層去猜
    return None
