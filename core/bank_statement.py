...
KEYWORD_RULES = [
    ...
    #
    # ── 貸款帳戶的固定結構（owner 2026-08-24：「一銀與合庫都是貸款帳號，
    #    結構都固定，你統一標準」）。一筆貸款在帳上的完整生命週期：
    #      撥款日：信保基金/手續費先扣 → 放款貸放進來 → 當天掃到營運帳戶
    #      之後每月：營運帳戶匯錢進來 → 放款本息/放款利息被扣 → 補貼息入帳
    #
    # 🔴 借款到位**只在放款行的那個帳戶**認列一次（放款貸放 → 貸款撥款）。
    #    掃到別的帳戶的那一筆是**轉存**，不是第二次借款 —— 兩邊都記成借款的話，
    #    負債會憑空變成兩倍，而且兩張報表都看起來很正常。
    ("放款貸放", "貸款撥款", +1),
    # 🔴 必須排在「利息」前面：`放款利息` 是**繳出去**的貸款利息，被
    #    「利息 → 銀行利息」攔截的話會變成利息**收入**（2026-08-24 實測到，
    #    一銀每月 1,850 + 2,498 全被記成收入）。
    ("放款利息", "貸款繳款", -1),
    ("放款本息", "貸款繳款", -1),
    ("攤還本息", "貸款繳款", -1),
    ("償還本息", "貸款繳款", -1),
    # 撥款當天先扣的兩種費用（信保基金＝中小企業信保的保證費）
    ("信保基金", "其他", -1),
    # 撥款當天把錢掃到營運帳戶
    ("寄庫寄款", "轉存", 0),
    ("貸款開戶", "轉存", 0),
