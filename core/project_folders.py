...
async def save_uploads(folder_abs: str, files, *, max_bytes: int,
    ...
    範圍：消費端是提案資產夾的兩個上傳入口（專案的與任一資料夾的）、
    企劃範本上傳（brief_templates）、以及 deck/報價單上傳（經
    proposal_assets.land_in_home）。政策（黑名單/清洗/去重）全在這裡；
    呼叫端只允許為了 HTTP 契約前置一道**大小**檢查（413 vs 這裡的 422），
    不准再前置副檔名檢查。影像紀錄**刻意沒接** —— 它走的是媒體副檔名
    白名單 + 時間戳前綴去重，是另一套政策，硬合併只會讓兩邊都變難懂。
    """
