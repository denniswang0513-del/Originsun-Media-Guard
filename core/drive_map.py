...
def map_drives_bat(mapping: dict) -> str:
    """把對應表變成一支 Windows 批次檔（owner 2026-09-14「有個地方可以下載一個 bat，幫我把這些路徑設好」）：
    每個字母先 `net use X: /delete`（舊的掛錯也重掛）再 `net use X: \host\share /persistent:yes`（重開機還在）。
    純 ASCII、CRLF —— cmd 對 UTF-8 中文很挑，註解一律英文；share 名有空白（storage 01）所以 UNC 一律加引號。"""
    lines = ["@echo off", "rem Originsun network drives - generated by Media Guard (drive_map)",
             "rem Re-run any time; existing mappings on these letters are replaced.", ""]
    for letter, unc in sorted(mapping.items()):
        if not unc:
            continue
        lines += [f"net use {letter}: /delete /y >nul 2>&1",
                  f'net use {letter}: "{unc}" /persistent:yes',
                  f'if errorlevel 1 echo   !! {letter}: failed ({unc}) - check the NAS is reachable and you are logged in', ""]
    lines += ["echo.", "echo Done. Current mappings:", "net use", "pause"]
    return "\r\n".join(lines) + "\r\n"
