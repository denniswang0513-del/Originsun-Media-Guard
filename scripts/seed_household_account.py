...
銀行轉帳付的 4 筆 22,100（owner 2026-08-26 拍板：代墊不分支付方式）＋同日缺欄
回填才歸進家用的刷卡列 11,256（鵝肉/金玉堂/全聯…—— Sheet 上同樣是未分類，
所以表公式也沒算到它們）→ 663,253。
...
async def run(apply: bool, prod: bool):
    ...
    try:
        ...
        if apply:
            print(f"家用往來推導餘額: {bal or 0:,}（預期 663,253 = 表公式 629,897 "
                  "+ 銀行付 22,100 + 回填新歸類 11,256）")
            if (bal or 0) != 663_253:
