...
## 現況 (v2.5.52) 基準線<!-- publish_update.py 自動維護版號，勿手改 -->
...
- [x] **Agent 離線告警**（v1.10.211，core/agent_watch.py）：master 每 5 分鐘巡機隊，
      連續 3 輪無回應推播、恢復也推播（⚠️ 生效前提：Google Chat webhook 要先填）

### 尚需完成

- [ ] 任務失敗率統計（成功率、平均處理時間）
- [ ] 獨立監控儀表板頁面（整合到主 UI；可併入 Work OS N4 管理者儀表板）
...
- [ ] 備份頁籤新增「選擇現有專案」下拉選項
- [x] 選擇專案後自動帶入 project_name、local_root、nas_root、proxy_root（2026-09-10 備份頁綁專案：`frontend/tabs/backup` 打 `/api/v1/projects/backup-roots`）
...
## Phase N：Work OS — 全公司作業系統（前期/後期/專案/工時/獎金/財務/官網 all-in-one）

**優先等級**：🔴 高 — 2026-07-07 定案的產品主線
**設計文件**：[`docs/WORK_OS_BLUEPRINT.md`](docs/WORK_OS_BLUEPRINT.md)（生命週期狀態機、
角色工作台、12 個盲點、owner 決策清單 — 動工前先讀它）
**依賴序**：N-now → N0 → N1 →（N2 ∥ N2'）→ N3 → N4。每段獨立可用、獨立 canary。

### N-now：結案 → 官網 一條龍（🔨 進行中 — 你目前的作業段）

把「專案結案 → 上架收件匣 → showcase 編輯 → AI SEO → 上架 → 對外站」走順。

- [x] 結案製作看板 + 上架收件匣（v1.10.202-204，顯式進場）
- [x] showcase 編輯器 AI 互動撰寫 + 參考資料上傳（v1.10.207-208）
- [x] 首頁 hero 挑選/精選網格/作品排序（v1.10.198-201）
- [x] **結案 → 通知官網編輯**（v1.10.220）：轉「結案作業」推播 project_closing
      （兩個狀態寫入點共用 `_notify_closing_entry`；webhook 設定後生效）
- [x] **上架驗收步驟**（v1.10.220）：rebuild 成功後實測對外 `/works/{slug}` 200
      → `website_verified_at` → 結案看板「已上線 ✓/驗證中」+ works_published 推播
- [x] 收件匣狀態欄位三態：經查已存在（待製作/製作中/已上線/不上官網 badge+篩選）
- [ ] 301 轉址收尾：legacy 轉址表 admin UI 卡片 + 舊站 46 個作品對照清單（owner 提供）
- [ ] 全鏈路實測一次：拿一個真實結案專案從看板走到對外站，記錄卡點

**做完你看到的**：結案後官網編輯自動收到通知，照收件匣做完按上架，系統自己
驗證對外站真的出現了 — 全程不需要工程師介入。

### N-soc：社群編輯自動化（可與 N0 並行；設計見藍圖 §2.5）
...
- [ ] n8n 發佈流（NAS 既有容器）：核准 → 發佈排程（黃金時段出手，非立即）→
      POST webhook 給 n8n → FB 粉專/IG/Threads 節點發文 → 回呼 published_url
      回寫佇列；workflow JSON 匯出存 repo（docs/n8n/）
- [ ] safety rails：每日/平台上限在發佈器強制、`social.autopublish` kill switch
      （關掉即退回一鍵複製模式）、發佈失敗告警 + 重試佇列
- [ ] **token 看門狗**：每日驗各平台 token，到期前 7 天推播提醒
- [ ] 影子運行兩週：先開「核准後排程但暫存不發」模式驗證產出品質，再開真發

**做完你看到的**：早上 Chat 收到「今日社群任務」，社群工作台改兩個字按核准 —
中午/晚上黃金時段貼文自動出現在粉專/IG/Threads，佇列裡躺著實際貼文連結。

### N0：地基 — 個人化 + 權限 scope + 通知到人 + 極簡審核（其餘全部依賴這段）

- [ ] **owner 決策**：同意個人帳號化（藍圖 §7-B — 工時/獎金/工作台的第一張骨牌）
- [ ] 每位同事 Google 帳號登入（OAuth 已就緒）、共用帳號降級為公用電腦工具用
- [x] User 加 `staff_id` 關聯人力庫（帳號 ↔ 人員檔案接起來）（v2.2.0 N0：`db/models/_system.py` 的 `User.staff_id`，使用者管理可綁）
- [ ] **row-level scope 機制**：守衛層加 own/team/all 維度（設計一次、
      工時/獎金/我的頁共用；別讓每個模組自己發明）
- [ ] **通知到人**：user 檔存個人 Chat DM webhook（或 @mention id），
      notifier 加收件人維度；公布欄支援指派到個人
- [ ] **極簡審核機制**：共用的「狀態+核可人+時間戳」小模組（工時核可/獎金簽核
      /請款簽核三處復用；不做 BPM 引擎）

### N1：專案生命週期主幹（藍圖 §1 — 所有領域掛在這條上）

- [ ] `crm_projects` 加 `phase` 欄（前期/拍攝/後期/審查/交付；`status` 不動保相容）
- [ ] 狀態機：每個轉換定義「誰能推」+「觸發什麼」（migration + 純函式 + 測試）
- [ ] PM 專案看板：phase 泳道視圖（進行中專案一眼看出卡在哪）
- [ ] 掛鉤 #3：開案 → 從報價項目自動生派工骨架
- [ ] 掛鉤 #5：備份完成 → 專案素材就緒 → 後期待辦出現
- [ ] 掛鉤 #9：狀態卡住 N 天 → 提醒對應角色（告警體系現成）

### N1-pre：前期資料資產（**無依賴、可先行** — 完整規劃見 [`docs/PREPROD_PLAN.md`](docs/PREPROD_PLAN.md)）

> 三個獨立資料資產，不依賴 N0/N1，可插在 N-now 收尾後任何空檔。建議順序 a → b → c。

- [x] **P-a 場景資料庫**（v1.10.221）：三表 + 照片上傳 + 使用履歷（評分+踩雷）
      → tab「🗺️ 場景庫」卡片牆+詳情 overlay。剩 P-a2 手機場勘頁 `/scout.html`（後補）
- [x] **P-b 提案資料庫**（v1.10.221）：win/loss 必填原因守門 + 共用參考片庫 +
      一鍵成案轉 CRM 專案 + 轉換率統計（by 類型/年度）→ tab「📑 提案庫」
- [x] **P-c 產業情報 runner**（v1.10.221，第四個 runner）：RSS 白名單（stdlib 零新
      依賴）→ claude 摘要分類評分+截止日 → tab「📡 產業情報」→ 一鍵轉提案。
      **enabled 預設 false — owner 加來源後手動開**；P-c2 Chat digest 依賴 webhook

### N2：工時鏈（藍圖 §2+§3）　∥　N2'：獎金政策（owner 作業，與 N2 並行）

> ⚠️ 2026-07-08 對齊現況：團隊已用 Google Sheet 填工時（小時制、專案時數預算、
> 手工消耗率%）— 粒度改**小時**、預算改**專案時數池**、內勤 UX 必須是 Sheet
> 同心智模型的列式 grid（切換成本趨近零），詳見藍圖 §3 現況修正段。

- [ ] 前置：專案加 `budget_hours` 時數預算欄（對齊 Sheet；「職務×人日」精細版後補）
      + H1 費率歷史
- [ ] **階段 0・Sheet 自動同步**（藍圖 §3.6）— **伺服器端已上線 v1.10.218**
      （timesheets 表 + `/api/v1/timesheets/ingest|summary|recent|ingest_token`，
...
- [ ] `timesheets` 表 + 預填鏈：排班 → 當日預填 → 本人手機一鍵確認（現場給
      整天/半天快捷鈕，落庫仍小時）
- [ ] 主管週結核可（用 N0 審核機制）→ 鎖定 → 日費快照 × 人日回寫專案成本
- [ ] **工時→專案檢核**（藍圖 §3.5，零額外填報 — 全是視圖層）：
      專案工時檢核卡（報價 vs 排班 vs 實際三條 burn bar + phase 對照）、
      超支預警（80% 提醒 PM / 100% 升級老闆，門檻可調）、
      人力負載週熱力圖（過載/閒置/派工缺口）、填報異常清單（漏填/無排班工時）
- [ ] N2'（owner）：獎金政策白紙黑字（藍圖 §7-A 六問）→ 試算表跑 3 個
      真實已結案專案驗證公式
- [ ] 影子運行一個月：與現行做法並跑對帳（藍圖 §7-L）

### N3：獎金系統（需要 N2 的實際工時 + N2' 的定案公式）

- [ ] 規則引擎：`core/crm_logic.py` 純函式 + 完整單元測試（薪酬零容錯）
- [ ] `bonus_rules`（版本化）+ `bonus_settlements` 表
- [ ] 觸發：收款完成 → 產結算「建議單」→ owner 簽核（N0 機制）→ 進請款流
- [ ] 我的獎金頁（本人只看自己 — 用 N0 scope）

### N4：角色工作台補完（藍圖 §5）

- [ ] user 加 `workspace` 欄：登入落地自己的工作台（管理者/業務/製片/現場/後期/財務/官網編輯）
- [ ] 「我的」聚合層：我的待辦 + 我的工時 + 我的獎金 + 我的通告（一支程式按登入者過濾）
- [ ] 管理者儀表板：專案管線 + 現金流 + 專案毛利（合約 − 外包 − 工時成本 − 雜支）+ 告警
- [ ] 現場人員手機工作台：今日通告 + 一鍵工時 + 雜支拍照 集成一頁

### N-biz：商業功能支線（穿插做 — 完整規劃見 [`docs/BIZ_PLAN.md`](docs/BIZ_PLAN.md)）

> 六項直接碰錢的功能，全部長在既有基礎設施上。除 B2 鎖 N2 後，其餘穿插 N 階段空檔。
> 優先 B3 → B1。

- [x] **B3 現金流預測 + 付款節點**（v1.10.220）：`payment_milestones` + 30/40/30
      模板 + 90 天週現金流視圖（帳務→💧現金流）+ 逾期應收警示；
      到期推播待 webhook 設定後掛每日提醒
- [x] **B1 看片審批客戶門戶**（v1.10.222 核心版）：proxy 送審 token 頁 `/review.html`
      + 時間軸留言 + 一鍵核准（video_path 不外洩、留言限流、重複核准 409）。
      剩 B1-2 交付下載區 + 保管年費（後補）
- [x] **B4 器材管理**（v1.10.222）：器材庫 + 領用歸還（重複 409）+ 保養 +
      月折舊攤提 + 稼動率報表 → tab「🎥 器材庫」
- [x] **B5 內部素材庫**（v1.10.222 核心版）：footage_index + pg_trgm 全文檢索
      「掃描資料夾」建索引（ffprobe metadata + 同名逐字稿）→ tab「🎞️ 素材庫」
      命中上下文高亮。剩：備份完成自動入索引 hook（後補）
- [ ] **B6 客戶關懷 runner**（第五個 runner，社群階段二穩定後）：結案 N 月回訪 +
      年度成效報告（續約觸發器）→ 三階包裝成客戶代營運服務線
- [ ] **B2 估價複盤迴圈**（🔒 N2 後）：結案自動比對報價 vs 實際（工時+雜支+外包）
      → 報價時「同類案歷史偏差 +N%」提示 — 把工時資料變成錢的關鍵一步

### N-hr / N-fin：員工管理 + 財務報表（完整規劃見 [`docs/HR_FIN_PLAN.md`](docs/HR_FIN_PLAN.md)）

> 邊界：N-hr 不做 HRIS（勞健保/薪資=會計師）、N-fin 不做記帳軟體（分錄=會計師），
> 只做管理決策層。⚠️ 兩個關鍵前置排序見文件末表。

- [x] **H1 員工檔案完整化**（v1.10.220 核心版）：僱用型態/到離職日/緊急聯絡人 +
      **`staff_rate_history` 費率歷史**（建檔種首筆、日費變動自動寫入 — N2/B2 前置
      已落地）；日費/時薪首次進 UI。剩：證照效期提醒/文件庫（後補）
- [x] **H2 請假假勤**（🔒 N0 審核機制）：極簡申請+核可+額度，不做打卡（`8572a72e` 一期＋2026-09-15 三步申請單 `HrLeaveApplication`＋時數帳；2026-09-16 再加彈性外出）
- [ ] **H3 派工適配**：技能篩選+檔期衝突（併入 N2 排班實作）
- [ ] **H4 協力評價**：結案 hook 順帶 1-5 星+註記（「誰靠譜」組織記憶，Lv3 可見）
- [ ] **H5 到離職 checklist**：模板→公布欄任務組；離職自動盤點器材/預支/派工
- [x] **F1 月結鎖帳**（v1.10.220）：`finance_month_close` + 鎖定月收支 409 守衛
      （實測 ✓）+ snapshot 留檔 + 重開稽核；月結卡在帳務→💧現金流。
      剩：科目表整理（F2 損益前一併做）
- [~] **F8 零用金請款**（2026-08-17，規劃見 [`docs/PETTY_CASH_PLAN.md`](docs/PETTY_CASH_PLAN.md)）：
      Google Sheet 的零用金帳戶搬進系統。**P0 匯入 + P1 員工自助已完成（dev 驗過，
      未發版、生產未匯入）**：獨立頁 `/petty-cash.html` 四分頁、支出單據擴充成
      「一筆登記同時餵專案成本與個人請款」、`crm_reimbursements` 批次、
      `finance_approve` 權限、own-scope 金額不受 `money_view` 抹除。
      剩 P2（核准／產 AP／匯款 CSV／逐行落帳）與 P3（專案「支出」分頁 + 樞紐）。
      🔴 **這批資料是 F2/F3 的前提** —— 生產 `crm_cash_entries` 至今 0 筆，
      三表全 0 的真因就是真帳還在 Sheet 裡（443 列、NT$507,791）。
- [ ] **F2 月度損益 + F5 應收帳齡**：收入−外包−雜支−固定成本、30/60/90 分桶
- [ ] **F3 專案毛利 + F4 客戶集中度**：初版現在能算（N2/B4 後自動變準）；前三大
      客戶占比 >50% 警示
- [ ] **F6 月報自動化 + F7 稅務準備包**：月結 runner 推 Chat（webhook 前置）、
      發票憑證年度打包給會計師；PDF 復用報價 PDF 管線

### 做完後你看到的改變

- 一個專案從洽談到獎金發放，每一步都有人負責、有頁面可做、有下一步自動出現
- 每個同事登入看到的是自己職務的工作台，不是 14 個 tab
- 老闆一頁看懂：哪個案子卡在哪、錢進來多少、該發多少獎金
...
現在 (v2.5.52) ← 你在這裡
    ...
    │
    ▼ Phase N-now: 結案 → 官網一條龍 (🔨 進行中)
    │   → ✅ 收件匣/AI撰寫/hero挑選 → 結案通知 → 上架驗收 → 轉址收尾 → 全鏈路實測
    │
    ▼ Phase N-soc: 社群編輯自動化 (可並行)
    │   → 第三個 AI runner：每日掃新內容 → 多平台文稿 → 審核卡 → 人工發布
    │
    ▼ Phase N0: 地基
    │   → 個人帳號化 → row-scope 權限 → 通知到人 → 極簡審核機制
    │
    ▼ Phase N1: 專案生命週期主幹
    │   → phase 欄+狀態機 → PM 泳道看板 → 掛鉤 #3/#5/#9
    │
    ▼ Phase N2: 工時鏈 (∥ N2' 獎金政策 owner 作業)
    │   → 排班日曆 → 通告單(手機) → 工時預填/確認/核可 → 成本回寫
    │
    ▼ Phase N3: 獎金系統
    ...
    ▼ ━━━ 支線（穿插在 N 階段之間做）━━━
    │
    ▼ Phase N1-pre: 前期資料資產（場景庫→提案庫→產業情報 runner，無依賴可先行）
    ▼ Phase N-biz: 商業功能線（B3 現金流→B1 審批門戶→B4 器材→B5 素材庫→B6 關懷 runner；B2 鎖 N2 後）
    ▼ Phase N-hr/N-fin: 員工管理+財務報表（⚠️ H1 費率歷史是 N2 前置、F1 月結是報表地基）
    ▼ Phase J-3: 備份 Tab 整合 (⬜ 小工程，隨時可插)
    ▼ Phase F-lite: 基礎監控 (🟡 → ✅ 失敗推播+Agent離線告警 v1.10.211 → 剩:失敗率+監控頁)
    ▼ Phase G: 多機多專案（第一步:佇列 DB 持久化）
    ▼ Phase L: 行動端適配（N2 通告/工時的手機頁會先吃掉大半需求）
    ▼ Phase C: Webhook → Phase D: MCP Server
    ▼ Phase E: master 去單點化（✅ 重建 SOP → NAS 災備 → Win11 → HTTPS）
    ▼ Phase F-full: 完整監控 (持續)
```

**下一步**：🔨 **Phase N-now（結案→官網一條龍）** — 你目前的作業段，清單見
Phase N 章節；收尾後進 **N0 地基**（第一件事：owner 拍板個人帳號化，
藍圖 §9 決策清單）。J-3 是小工程，任何時候想換口味可以插隊做掉。
