...
- [ ] **結案 → 通知官網編輯**：專案轉「已結案」時推播「收件匣有新作品」
      （告警體系已就位，加一個 notify hook + 模板即可）
- [ ] **上架驗收步驟**：作品上架 rebuild 後，自動檢查對外頁回 200 + 出現在
      作品列表，結果回寫收件匣（「已上架 ✓」才算閉環，不是按下核准就結束）
- [ ] 收件匣狀態欄位補齊：待處理/製作中/已上架 三態可視（若已有則勾掉）
...
- [ ] **B3 現金流預測 + 付款節點**（可立即插隊）：`payment_milestones` 綁 phase/日期
      → 90 天現金流視圖（應收+應付+固定成本）→「該請款了」提醒（忘記請款是常態）
...
- [ ] **H1 員工檔案完整化**：僱用型態/技能證照(效期提醒)/文件庫 +
      **`staff_rate_history` 費率歷史 — N2 成本回寫與 B2 複盤的共同前置，N2 前必須落地**
...
- [ ] **F1 科目表 + 月結鎖帳**（N-fin 地基，最先做）：鎖定月不可改+audit、月結檢核
