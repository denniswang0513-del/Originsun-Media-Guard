...
- ✅ OTA 純 HTTP 熱更新 + 版本守護（防版本倒退）+ 更新防卡死（30s fallback + 5min timeout）
- ✅ 語音生成 Tab — Edge-TTS + F5-TTS 聲音複製 + NAS 聲音庫 + 台灣正音引擎
- ✅ 專案總覽 — 單行緊湊任務卡片 + 歷史分頁 + 佇列管理 + 排程中 badge
- ✅ 運算主機合併至機器狀態 — NAS 共享 agents.json，各 TAB host checkbox 即時同步
...
- [ ] **響應式佈局**：手機/平板斷點（768px / 1024px），頁籤改為底部導航或漢堡選單
...
**下一步**：Phase H 剩餘（TTS 佇列整合）或 Phase K（遠端一鍵更新 Agent）。
