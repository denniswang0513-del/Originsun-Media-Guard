...
## 現況 (v1.10.211) 基準線<!-- publish_update.py 自動維護版號，勿手改 -->
...
AI 每日規劃社群任務、產多平台文稿，人只審核與發布。骨架 = 第三個 AI runner
（同 AI SEO / 翻譯的「排程 claude 產出 → 後台審核卡 → 核准」模式）。

- [ ] **owner 拍板**（藍圖 §2.5 五問）：平台清單與帳號、V1 人工發布（建議）、
      頻率上限、客戶授權規則（public 即可宣傳？）、runner 排程時段錯開
- [ ] `social_posts` 表 + `website_settings` social.* 鍵（平台模板/頻率/水位）
- [ ] `services/website/social_runner.py`：每日掃新內容（作品/文章/公益）→
      無新內容走常青輪播 → 每選題 × 每平台產文稿（平台語氣模板 + 品牌調性 +
      官網文案當素材 + 建議配圖）→ 入佇列 draft
- [ ] 每日摘要推播「今日社群任務：N 篇待審」+ 公布欄指派官網編輯
- [ ] admin 子視圖「📣 社群工作台」：佇列 + 審核卡（改稿/核准/退回）+
      一鍵複製（文稿+配圖連結）+ 已發布標記（貼實際連結）
- [ ] 平台語氣模板後台可編（IG 短+hashtag / FB 敘事 / Threads 口語）
- [ ] 常青輪播去重（同作品 N 個月內不重推）+ 頻率上限守衛
- [ ] 影子運行兩週：AI 每日產、編輯實際用，校準語氣模板後才算穩定

**做完你看到的**：每天早上 Chat 收到「今日社群任務」，打開社群工作台改兩個字
按核准、複製貼上發文 — 社群經營從「想內容」變成「審內容」。
（V2 選配：平台 API 自動發布 — 等 V1 跑順且明確需要再評估，token/帳號風險高。）
