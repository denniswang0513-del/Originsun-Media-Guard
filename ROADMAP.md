...
- [ ] **P-a 場景資料庫**：`preprod_locations` + 照片 + 使用履歷（專案連動+踩雷紀錄）
      → 卡片牆 tab「🗺️ 場景庫」→ P-a2 手機場勘頁 `/scout.html`（現場拍照即入庫）
- [ ] **P-b 提案資料庫**：`preprod_proposals`（win/loss 必填原因）+ 參考片庫
      → 一鍵成案轉 CRM 專案 → 提案→成案轉換率卡（STRATEGY 指標落地）
- [ ] **P-c 產業情報 runner**（第四個 AI runner，複用 `_runner_util` 骨架）：
      白名單來源（政府採購網標案/文化部文策院補助/產業媒體 RSS）→ 每日 08:30
      抓取+claude 摘要分類評分+截止日 → tab「📡 產業情報」→ 高分標案一鍵轉提案
      （閉環：情報→提案→專案→結案→官網→社群）；P-c2 Chat digest 依賴 webhook 設定
...
- [ ] **B1 看片審批客戶門戶**（N-now 後最優先）：proxy 送審 token 頁 + 時間軸留言
      + 一鍵核准 → B1-2 交付下載區 + **素材保管年費**（recurring revenue）
- [ ] **B4 器材管理**：器材庫 + 領用歸還 + 保養 + **折舊攤進專案成本**（毛利去虛）
      + 稼動率報表（買 vs 租有據）
- [ ] **B5 內部素材庫**：既有 Whisper 逐字稿 + metadata + 縮圖條 → pg_trgm 全文檢索
      「找所有海岸空拍」；與素材進線自動化合流實作
