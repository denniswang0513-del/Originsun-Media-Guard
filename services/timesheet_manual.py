...
async def project_options(session, staff_name: str | None = None) -> list:
    """補登用專案下拉＝共用的清單規則 ＋ 該員最近填過的專案名。

    清單規則（不篩狀態、母私帳去重、最近有動的排前面、label／closed）住在
    `services/project_picker.py`，備份頁的「綁定專案」用的是**同一支** ——
    owner 2026-09-11：「綁定專案的列法和專案工時的專案列表相同」「不篩，連專案工時也不篩」。
    這裡原本有 `status.in_(("製作","結案"))`，結案後要補工時的人選不到案子。
    """
    from db.models import Timesheet
    from services.project_picker import list_options
    opts = await list_options(session)
    if staff_name:
