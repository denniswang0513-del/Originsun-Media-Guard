...
### 9.2.2 研究方向（owner 2026-09-18：「研究主題找的方向可以設定嗎」）

原本方向只能靠結論／筆記／標籤間接推，沒有一句話可以直接說「往這邊找」。加了兩件事：

1. **每本書一格「研究方向」**（`meta.focus`，最多 `FOCUS_MAX`＝300 字）。
   在提示裡排在**結論之前**，寫明「跟下面衝突時以這段為準」。空的就照舊自己判斷。
   端點走既有的 `PUT /{id}`（多收一個 `focus`）；畫面在「延伸」分頁，平常只是一行字＋一顆「改」。
2. **評分回饋進下一次搜尋**。2026-09-18 之前「有用／沒用」只決定討論要不要帶那一則，
   **下一次搜尋完全看不到** —— 同一類東西還是會再找回來（只是換個網址）。
   現在各帶最近 `EXTEND_EXAMPLES`（8）則的標題與出處當正反例。沒評過的不算偏好，不進提示。

**實測（dev 8001，2026-09-18）**：同一本書，沒設方向那一輪回來九則裡有六則是英文的美國內容
（維基、Ben Carlson 談稅、arXiv 論文、4% 法則、Empower 的美國緊急預備金調查）。
把方向設成「只找台灣本地的稅務、勞健保、中小企業與接案公司現金流，不要美股與美國退休金制度」之後，
新的六則**全部**是繁中的台灣本地資料：財政部的營所稅暫繳期限、主計總處家庭收支調查、
2027 勞保費率調升到 13%、最低工資審議、勞退自提率、央行對傳產中小企業的放款。方向確實吃得進去。
...
## 9.7 章節裡的圖與表（owner 2026-09-18：「如果章節有重要圖片 或表格 我希望你也可以截取出來」）

兩者走**不同的路**，因為 claude 讀得到字、看不到圖：

| | 怎麼抽 | 存哪 | claude 怎麼用 |
| --- | --- | --- | --- |
| 表 | `page.find_tables().to_markdown()` | **直接塞進 `full_text.txt`**，標 `[[表 p.N-k]]` | 切章時跟著走，看得到內容，用得到就照抄進「框架」或「實例」 |
| 圖 | `page.get_images()` ＋ `doc.extract_image()` | `<書>/assets/pNNN-k.jpg` | 🔴 **看不到圖**；提示只給頁碼與圖旁邊的字，由它決定要不要寫 `![說明](assets/…)` |

**什麼叫「重要」的圖**（`knowledge_logic.keep_image` ／ `keep_page_image`）

實測「台灣菜」503 頁：原本抽出 71 張，其中 13 張是封面與整頁掃描 —— 那幾頁**本文 0 字**、
圖佔 52%～100% 版面，也抓不到圖說。加上兩條判準之後收斂成 50 張，**每一張都有圖說**
（「圖2.1 日治初期台北瑞成春酒樓的菜牌」那種）：

1. 兩邊都要 ≥ 200px（擋掉頁首小圖示，那本書裡有一張 40x40）。
2. 那一頁本文要 ≥ 80 字，且圖不能佔掉 90% 以上版面（擋掉封面與整頁掃描）。

圖說取圖**下方** 80pt 內的文字（`_caption_near`）—— 實測中文書的圖說都在圖下面。

**私有的圖怎麼進畫面**

`<img src>` 送不了 Authorization，所以：md-lite 只產 `<img data-md-src="assets/…">`（**故意不給 src**，
它也不該知道怎麼取檔；順帶的好處是永遠不會自己連外），書頁 `_fillAssets` 帶 token fetch 成 blob 再填，
重畫前 `revokeObjectURL` 收回。圖說（claude 寫在 alt 裡）另外畫一行出來 —— alt 只有讀螢幕看得到。
端點走 `core.no_store`（書是私有的，圖也是）。

**限制**

- 補抽（`ensure_assets`，編譯前跑）**只抽圖，不重寫 `full_text.txt`**。表格是上傳時塞進全文的，
  舊書要有表格就重新上傳一次。
  （2026-09-18 實測：重寫全文會把現在那一份整個蓋掉 —— 連「這本抽不出文字」的狀態都蓋掉，
  掃描檔因此變成編譯成功。有測試釘住。）
- 只收 png／jpg。svg 會夾帶 script，其他格式手機不一定開得了。
- 一本最多 80 張、一頁最多 4 張。

## 9.8 書籍資訊、圖輯與圖說（owner 2026-09-19）

**「我希望有個 tab 可以填書的基本資訊」** → 新的「資訊」分頁，十個欄位存在 `meta.info`
（副書名／原文書名／譯者／出版社／出版年／版次／ISBN／連結／我為什麼讀這本／讀完日期）。
白名單與長度在 `knowledge_logic.normalize_info`；`link` 只收 `http(s)`（那一格會被畫成可點的連結）。
前端 `info.js` 的欄位順序與鍵由測試比對，少一個那一格就永遠存不進去。

**「標籤可以很多個」** → `TAGS_MAX` 10 → 24。

**「圖片擷取很重要…可以在文章底部用相簿、圖說方式（標注頁數）呈現」**

🔴 這一條推翻了 §9.7 原本的做法。原本靠提示叫 claude 自己在文章裡插 `![](assets/…)`，
實測三本真書：台灣菜 50 張只引用 3 張、冰與血之歌 10 張（圖說都很完整）引用 0 張、
鯨豚 35 張引用 0 張。**靠模型挑，大部分的圖永遠不會出現。**

現在由程式決定：
- **章末相簿**：那一章頁碼範圍內的**每一張**，附圖說與頁數。
- **「圖輯」分頁**：整本書的圖，照章分組（`_chapter_rows` 因此多回 `start_page`／`end_page`）。
- 提示還是請它「能插入的就插入」（owner 同日補充），但插不插都不影響相簿 ——
  文中插過的**照樣**列在相簿，相簿是完整索引不是「剩下的」。

**「可以製作圖說」** → 從 PDF 刮下來的圖說常黏著正文
（「圖2-1：卑南社與馬蘭社位置圖 資料來源：…163 三」尾巴那個「163 三」是頁碼與下一段開頭）。
章節那一發多要一段 `## 圖說`（`檔名｜一句話`），產完由 `parse_captions` 取走存進 `meta.asset_captions`，
不留在文章裡；模型沒寫到或寫錯檔名的沿用刮下來的原文，圖說不會變空。**不多花一次呼叫。**

**「要標明出處」** → 每張圖都標「出自《書名》作者，第 N 頁」（`ctx.figureSource`）。

## 9.9 公開分享一本書的研究（owner 2026-09-19）

owner：「可以有一個公開分享的連結，讓我把這本書的研究分享出去（**但是點不到我其他的地方**）」。

| | |
| --- | --- |
| 開關 | 「資訊」分頁最下面；`PUT /{id}/share {on}`（私有，過 `_guard`） |
| 網址 | `https://foundry.originsun-studio.com/share.html#<32 hex>` |
| 公開端點 | `GET /api/v1/knowledge/public/{share_id}` —— **整個知識庫唯一不需要登入的** |
| 公開頁 | `frontend/share.html`，樣式與程式全部內嵌（所以也不吃 §9.6 那個快取問題） |

🔴 **公開的那一面獨立成 `routers/api_knowledge_public.py`**：想知道「哪些東西是公開的」看那一個檔就夠了。
混進 `api_knowledge.py` 的話，那支「每一支端點都要 `_guard`」的測試就有了例外，以後誰忘了守都不會被抓到。

**外面看得到的就這些**：書名、作者、`meta.info`、標籤、延伸（去掉他評成「沒用」的）。
**看不到**：筆記、結論、討論、章節重點、骨架、全文、書裡的圖 ——
前三樣是他自己的東西，後四樣是原書的內容（公開有版權問題）。

其餘的規矩（每一條都有測試）：分享 id 是 32 hex 隨機、跟書 id（16 hex）刻意不同長度，
書 id 不能當分享網址用；沒開分享跟不存在都回同一句 404；只有 GET；不提供列表；
關掉就把 `meta.share` 整個拿掉，再開會換一組新 id（舊連結立刻失效）；
分享 id 放在 hash（不進伺服器存取紀錄、不會被 referer 帶走）；頁面 `noindex, nofollow`；
外連一律 `noopener noreferrer nofollow`；頁尾寫明「書本身的內容不在這裡，要看原書請支持原作者與出版社」。
