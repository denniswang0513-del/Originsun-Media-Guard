...
## 9. 定期收錄這本書的網路資料（owner 2026-09-18：「也可以定期收錄整理這本書相關的網路資料」）→ v1.5 後段
...
```
books/<id>/
  延伸.md          目前的「延伸索引」：每則一行 —— 日期｜標題（連結）｜出處｜一句摘要｜關聯章｜對他的意義｜他的評分（有用／沒用／未評）
...
2. **定期自動**：每本書 `meta.watch: true` 的（預設 **false**）＋全域 `finance.knowledge_watch.enabled`（預設 **false**，同 `intel.enabled`／`social.enabled` 的規矩：owner 手動開，防 claude 額度）。
   ...
   「這本書（書名／作者／骨架的核心框架 5 條／他的結論）最近 30 天有哪些值得看的：作者新文章／訪談／podcast 文字稿、對這本書框架的實際應用或批評、跟他情境（台灣、影像製作公司負責人、私帳）相關的討論。繁中＋英文各搜；回 JSON 陣列；**已收錄過的網址**（給清單）不要再回；每則附出處與一句『為什麼值得他看』；找不到就回空陣列，不要湊。」
