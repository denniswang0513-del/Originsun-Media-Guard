...
一個獨立頁 `/m/crm.html`、底部五個分頁（**發票／零用金**／專案／報價／**行事曆**（owner 2026-09-03：取代付款；付款退成隱藏路由，從專案抽屜「付款清單」進；規劃 `docs/SHOOT_CALENDAR_PLAN.md`）；owner 2026-09-03：發票與零用金最常用，排最前面；零用金直接掛既有 `tabs/petty/petty-view.js`，不另做），共用一個殼模組；
...
- `crm.html` + `crm.js`：深色（`#1a1a1a`／`#2a2a2a` 卡片／`#3b82f6` 主色，同 `/expense.html`），`maximum-scale=1`，底部 tab bar：
  ...
  - **發票**＝「請同事開票」（owner 2026-09-03 定案）：手機登記一張待開立的請款發票，順序 專案（必選，自動帶客戶／抬頭／統編／名稱）
    →類別（專案／內部代開／外部代開）→申請人（設定檔那份名單，手機記住上次選的）→抬頭（打字找客戶全稱，自動帶統編）→品項→電子或紙本→金額；
    其餘自動：方向＝收款、款項狀態＝未收、開立狀態由後端看號碼（沒有＝未開立）。送出後產生可複製的通知（LINE 貼給同事開票）＋最近 10 張。
    長清單一律「打字就過濾」的選擇器（`ui.mountPicker`），短清單用分段按鈕；字彙全部來自 `/options`。
  ...
  - **行事曆**（2026-09-03，取代付款）：拍攝排程＋器材登記，見 `docs/SHOOT_CALENDAR_PLAN.md` §3。
  - **付款**退成隱藏路由 `#payments`（專案抽屜「付款清單」進）；**記雜支**也是隱藏路由 `#expense`（專案抽屜原地展開）。
