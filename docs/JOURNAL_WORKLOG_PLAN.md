...
## 14. v7（owner 2026-09-05 拍板與修正）

- **求助條目：大家都看得到**（§7 第 4 題拍板）。「需要協助」「想討論」的條目在「大家的回顧」照常顯示，主管回覆也全員可見。
- **工作階段設定按鈕**：專案紀錄格子右下「工作階段設定」→ 每個分類一格，改名、上下排序、新增、停用不刪；儲存後新列的階段下拉立即更新。
  正式版＝CRM 設定分頁同一個編輯器（存 `work_stage_nodes`），這顆按鈕對有 crm 管理權的人才顯示。
- **示範頁標註**：還沒有的功能一律標「開發中」（領用器材入口、團隊的一週、專案查詢開放員工、從場次／待辦帶入、工作階段欄、
  上週做了什麼自動區、掛案子、求助標記、主管回覆、草稿／送出、大家的回顧展開版）。沒標的＝既有功能搬位置。
...
**2026-09-05 追加拍板**：主管回覆的即時通知**先拿掉**（回覆只在頁面上看到）。要加回來時走個人通道（D3 通知偏好），不用全域 Chat webhook 廣播。

## 16. 2026-09-05 追加（owner）：不用回覆，改心情；大家的回顧不放「做了什麼」；視覺照示範頁

- **回覆框拿掉**，改成每條可按心情：讚／愛心／笑（`journal_reactions`，一人一條一種只有一筆、再按取消；只能對送出的按；
  `POST /journal/react`，mine／week／person 每份都帶 `reactions`）。這三個 emoji 是「UI 無 emoji」鐵則唯一的例外，owner 指名。
  後端 `/reply`、`/help` 端點保留（沒有 UI 用它）。
- **大家的回顧只看四問**，「做了什麼」自動區只在自己那份。
- 週記頁視覺對齊示範頁：按鈕圓角 8px、問題標題粗體、textarea 虛線圓角、人列摘要（第一條的前 40 字）。
