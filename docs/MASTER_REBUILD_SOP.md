...
> 📅 最後更新：2026-07-07　—　🔴 規格與程式碼分歧時**以程式碼為準**。
> 這份文件不會自動跟著改，日期越舊越要當心（用 `git log -1 -- docs/MASTER_REBUILD_SOP.md` 確認）。
...
⚠️ **OS 生命週期**：master 現跑 Windows 10 Pro 22H2（2026-07 盤點）——
消費者支援 **2025-10-14 已終止**，ESU 最多延到 **2026-10-14**。
在那之前要排 Windows 11 升級（或換機）；重建時（§2）直接裝 Windows 11。
