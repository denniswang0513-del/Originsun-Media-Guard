...
- **P3**：衝突待決、未對映、類似專案、建議預算套用（§4 的 2、6、8、9）。✅ 2026-09-12（錢四欄＋套用建議預算＋未對映指定＋
  專案檔案的加入比較／改預算／匯出＋團隊的一週「衝突待決 N」；§4-2 的格子案名旁燒率 chip 未做——格子是共用元件，等 owner 看過再決定要不要）
- **P4**：拿掉舊的 today（今日看板）／mine／projects／staff 四個 view；兼職排班獨立視窗保留。✅ 2026-09-12（timesheets.js 1,221→889 行；
  留 總表／儀表板／設定／並排比較／專案檔案頁（儀表板點進來）；八條釘舊 view 的測試改釘 ts-zone）
