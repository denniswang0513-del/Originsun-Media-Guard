...
- 第 15 輪：後期代開母帳合約額 0 時建的分身（份額 0），owner 在私帳直接打面額 90,000 → 之後母帳填 95,000 再重同步
  → 份額 0→95,000、owner 的 90,000 留在「自己那部分」→ X 185,000。系統分不出那 90,000 是面額還是 owner 加的；
  後期代開的面額請填在**母帳**，不要打在私帳。繼承自 X 的代開份額用份額上的 `face` 標記分辨：標了（舊 1:1 代開整筆認、切過後期代開、
  母帳自己走後期代開）＝發票面額、重同步只更新工項；沒標（N:1 回填的成本行）＝跟成本行走。換回時 `face` 清掉。

## 後期代開母帳案的成本歸私帳（owner 2026-09-13 晚）

公司案改成後期代開＝整案其實是私帳主人的。逐案損益**讀取時**（`_crm_costs`／`_crm_lines`，不落庫、同 CRM_BACKED 那條）把
連到私帳案 X 的後期代開母帳案 P 的 CRM 帳目算進 X：P 的行政雜支（沒跟公司請款的）→ X 的雜支；P 的人員費用中**不是私帳主人**
的那幾行（`actual_staff_id`／`estimated_staff_id` ≠ `mine_owner_staff_id`）→ X 的委外；主人自己那幾行是收入（鏡射成工項）不算。
明細列出來標「來自 P（後期代開）」。認不出主人（沒有唯一的 finance_mine 帳號）→ 整案人員費用都算委外。
