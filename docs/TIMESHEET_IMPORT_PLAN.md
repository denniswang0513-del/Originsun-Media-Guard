...
1. **撞案 52 個**：等 dry-run 報告逐一指定（多數是媒體顧問月費系列）—— 或先接受 §4 規則 2 能解的，其餘暫留未對映
2. **4 個沒建檔的人**（林依靜、郭昭君、陳妍蓁、黃柔云，319 列）：建 `crm_staff` 或留名字
3. **「源日後期_*」5 個（326h）**：當內部桶，還是建一個私帳案「源日後期」收
4. **預算是否一併灌**（367 案）：建議灌 —— 執行中 33 案的 Burn 立刻有意義；已結案的也留著給 B2
